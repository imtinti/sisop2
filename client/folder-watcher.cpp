...
// private
void FolderWatcher::inotify_init() {
...
void FolderWatcher::inotify_add_watch() {
...
FolderWatcher::FolderWatcher(const char* path) : path(path) {}
        ...
        for (bufferPointer = buffer; bufferPointer < buffer + length;) {
            ...
            if (isValidEvent) {
                if (event->mask & IN_MOVED_FROM) {
                    ...
                    std::cout << "file '" << event->name
                              << "' was moved to the directory" << std::endl;
                } else if (event->mask & IN_CREATE) {
                    std::cout << "file '" << event->name << "' was created"
                              << std::endl;
                } else if (event->mask & IN_MODIFY) {
                    std::cout << "file '" << event->name << "' was modified"
                              << std::endl;
                }
            }
